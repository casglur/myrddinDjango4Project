...
def Poem(request, myrddin_id): 
    ...
    versions_dict_data = xmltodict.parse(versions_response.content)
    versions_json_object = json.dumps(versions_dict_data, indent = 4)
    versions_json_object = json.loads(versions_json_object)
    versions_json = versions_json_object['exist:result']['exist:ids']['exist:id']
    versions_json_dumps = json.dumps(versions_json, indent = 4)
    versions_json_loads = json.loads(versions_json_dumps)
    versions_list = versions_json_loads
    ...
    context = {
        ...
        'request': request,
        'versions_json_dumps': versions_json_dumps,
        'versions_json_loads': versions_json_loads,
